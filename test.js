const config = {
  prefix: '!!',
  token: 'NTc3MDU1MjYxNzg1NzE4Nzkz.XcWNJw.w8FNHgo46XrCeVvg9-KrIlQ1Ak0',
  nodes: [
    { host: '192.168.0.20', port: 2333, password: 'youshallnotpass', reconnectInterval: '1000', reconnect: true }
  ]
}
...
client.on('message', async msg => {
  ...
  if (command === 'play') {
    if (!msg.member || !msg.member.voiceChannel) return msg.reply('You must be in a voice channel for this command.')

    const track = args.join(' ')
    const [song] = await getSongs(`${track}`)
