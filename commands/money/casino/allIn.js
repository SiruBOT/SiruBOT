class Command {
  ...
  async run (compressed) {
    ...
    if (GlobalUserData.money < 10000) {
      message.reply(picker.get(locale, 'COMMANDS_CASINO_ALLIN_UNDER'))
    } else if (GlobalUserData.money <= 0) {
      message.reply(picker.get(locale, 'COMMANDS_CASINO_ALLIN_MONEY_ZERO'))
    } else {
      if (!message.author.isCasino) {
          ...
          .then(() => {
            ...
            setTimeout(async () => {
              ...
              if (result === true) {
                ...
                const randFailMessage = this.client.utils.randmizer.chooseWeighted(picker.get(locale, 'COMMANDS_CASINO_ALLIN_RAND_FAIL', { RANDMESSAGE: loadMessage, MEMBER: message.member }), [60, 15, 10, 14, 1])
        ...
        message.reply(picker.get(locale, 'COMMANDS_CASINO_OTHER'))
      }
    }
  }
}
