class Command {
  ...
  async run (compressed) {
    ...
    if (['none', '없음', 'null', 'remove', '지우기'].includes(args.join(' ').toLowerCase())) {
      ...
      this.client.database.updateGuildData(message.guild.id, { $set: { vch: '0' } })
    } else {
      ...
      const options = {
        ...
        formatter: formatter,
        collection: message.guild.channels.filter(el => el.type === 'voice'),
        filter: filter,
        message: message,
        locale: locale,
        picker: picker
      ...
      this.client.utils.findUtil.findElement(options).then(async (res) => {
        ...
        this.client.database.updateGuildData(message.guild.id, { $set: { vch: res.id } })
      })
    }
  }
}
